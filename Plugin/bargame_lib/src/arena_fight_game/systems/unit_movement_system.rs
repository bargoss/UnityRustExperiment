    ...
    for (unit, mut character_movement, position) in unit_query.iter_mut() {
        ...
        match unit.following_entity {
            ...
            Some(following_net_id) => {
                ...
                let dir_towards_target = delta / (delta_mag + FP::new(0.0001)); // prevent divide by zero
                ...
                let mut follow_component = FP::new(1.0);
                let mut orbit_component = FP::new(0.0);


                if delta_mag < FP::new(2.5) {
                    follow_component = FP::new(0.0);
                    orbit_component = FP::new(1.0)
                }
